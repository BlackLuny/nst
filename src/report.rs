use crate::{Metrics, NetworkTestError, Result};
use chrono::Utc;
use std::fs;
use std::path::Path;
...
impl Report {
    ...
    fn generate_text(&self) -> String {
        ...
        if let Some(duration) = self.metrics.get_test_duration() {
            text.push_str(&format!("Total Duration: {duration:?}\n"));
        }

        text.push('\n');

        if let Some(overall_score) = self.metrics.overall_score {
            text.push_str(&format!("OVERALL SCORE: {overall_score:.1}/100\n"));
        ...
        if let Some(ref bandwidth) = self.metrics.bandwidth {
            ...
            text.push_str(&format!(
                "Average Upload Speed: {:.2} KB/s\n",
                bandwidth.average_upload_speed / 1024.0
            ));
            text.push_str(&format!(
                "Average Download Speed: {:.2} KB/s\n",
...
"#,
                tcp.stability_score, tcp.uptime_percentage
            ));
        }
...
"#,
    ...
    fn save_to_file(&self, content: &str, file_path: &str) -> Result<()> {
        if let Some(parent) = Path::new(file_path).parent() {
            fs::create_dir_all(parent).map_err(NetworkTestError::Io)?;
        }

        fs::write(file_path, content).map_err(NetworkTestError::Io)?;
