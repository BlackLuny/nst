...
impl Metrics {
    ...
    pub fn get_summary(&self) -> MetricsSummary {
        MetricsSummary {
            ...
            network_quality_score: self.network_jitter.as_ref().map(|n| n.network_quality_score),
        }
    }
    
    fn count_tests_run(&self) -> usize {
        let mut count = 0;
        if self.tcp_stability.is_some() { count += 1; }
        if self.bandwidth.is_some() { count += 1; }
        if self.connection_perf.is_some() { count += 1; }
        if self.dns_stability.is_some() { count += 1; }
        if self.network_jitter.is_some() { count += 1; }
        count
    }
    ...
    pub fn export_csv(&self) -> String {
        ...
        csv.push_str(&format!("session,proxy_address,{},\n", self.proxy_config.proxy_address));
        
        if let Some(score) = self.overall_score {
            csv.push_str(&format!("overall,score,{:.2},points\n", score));
        }
        
        if let Some(ref tcp) = self.tcp_stability {
            csv.push_str(&format!("tcp_stability,stability_score,{:.2},points\n", tcp.stability_score));
            csv.push_str(&format!("tcp_stability,uptime_percentage,{:.2},percent\n", tcp.uptime_percentage));
            csv.push_str(&format!("tcp_stability,average_rtt,{},ms\n", tcp.average_rtt.as_millis()));
            csv.push_str(&format!("tcp_stability,reconnections,{},count\n", tcp.reconnections));
        }
        
        if let Some(ref bandwidth) = self.bandwidth {
            csv.push_str(&format!("bandwidth,bandwidth_score,{:.2},points\n", bandwidth.bandwidth_score));
            csv.push_str(&format!("bandwidth,average_upload_speed,{:.2},bytes_per_sec\n", bandwidth.average_upload_speed));
            csv.push_str(&format!("bandwidth,average_download_speed,{:.2},bytes_per_sec\n", bandwidth.average_download_speed));
            csv.push_str(&format!("bandwidth,connection_interruptions,{},count\n", bandwidth.connection_interruptions));
        }
        
        if let Some(ref conn_perf) = self.connection_perf {
            csv.push_str(&format!("connection_perf,performance_score,{:.2},points\n", conn_perf.performance_score));
            csv.push_str(&format!("connection_perf,success_rate,{:.2},percent\n", conn_perf.success_rate));
            csv.push_str(&format!("connection_perf,average_connection_time,{},ms\n", conn_perf.average_connection_time.as_millis()));
            csv.push_str(&format!("connection_perf,max_concurrent_successful,{},count\n", conn_perf.max_concurrent_successful));
        }
        
        if let Some(ref dns) = self.dns_stability {
            csv.push_str(&format!("dns_stability,dns_score,{:.2},points\n", dns.dns_score));
            csv.push_str(&format!("dns_stability,success_rate,{:.2},percent\n", dns.success_rate));
            csv.push_str(&format!("dns_stability,average_query_time,{},ms\n", dns.average_query_time.as_millis()));
            csv.push_str(&format!("dns_stability,timeout_rate,{:.2},percent\n", dns.timeout_rate));
        }
        
        if let Some(ref jitter) = self.network_jitter {
            csv.push_str(&format!("network_jitter,network_quality_score,{:.2},points\n", jitter.network_quality_score));
            csv.push_str(&format!("network_jitter,packet_loss_rate,{:.2},percent\n", jitter.packet_loss_rate));
            csv.push_str(&format!("network_jitter,average_rtt,{},ms\n", jitter.average_rtt.as_millis()));
            csv.push_str(&format!("network_jitter,jitter,{},ms\n", jitter.jitter.as_millis()));
        }
...
impl MetricsSummary {
    pub fn print_summary(&self) {
        ...
        if let Some(score) = self.overall_score {
            println!("Overall Network Stability Score: {:.1}/100", score);
        ...
        if let Some(score) = self.tcp_stability_score {
            println!("  TCP Stability: {:.1}/100", score);
        }
        if let Some(score) = self.bandwidth_score {
            println!("  Bandwidth: {:.1}/100", score);
        }
        if let Some(score) = self.connection_perf_score {
            println!("  Connection Performance: {:.1}/100", score);
        }
        if let Some(score) = self.dns_stability_score {
            println!("  DNS Stability: {:.1}/100", score);
        }
        if let Some(score) = self.network_quality_score {
            println!("  Network Quality: {:.1}/100", score);
        }
        println!();
    }
}
