...
impl DnsStabilityTest {
    pub fn new(proxy_addr: &str, domains: Vec<String>, query_interval_ms: u64, test_duration_sec: u64) -> Self {
        Self {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("Query interval: {:?}, Test duration: {:?}", self.query_interval, self.test_duration);
    ...
    async fn run_dns_test(&self, client: &Socks5Client) -> Result<DnsStabilityResult> {
        ...
        for domain in &self.domains {
            domain_results.insert(domain.clone(), DomainResult {
                domain: domain.clone(),
                total_queries: 0,
        ...
        for domain_result in domain_results.values_mut() {
            if !domain_result.query_times.is_empty() {
                domain_result.average_query_time = domain_result.query_times.iter().sum::<Duration>() 
        ...
        let min_query_time = all_query_times.iter().min().copied().unwrap_or(Duration::ZERO);
        let max_query_time = all_query_times.iter().max().copied().unwrap_or(Duration::ZERO);
    ...
    async fn perform_dns_query(&self, client: &Socks5Client, domain: &str) -> Result<Duration> {
        ...
        match query_result {
            ...
            Ok(Err(e)) => Err(e),
            Err(_) => Err(NetworkTestError::Timeout(format!("DNS query timeout for {}", domain))),
        }
    }
    
    async fn dns_query_via_proxy(&self, client: &Socks5Client, dns_server: &str, domain: &str) -> Result<()> {
        let mut stream = client.connect(dns_server).await
            .map_err(|e| NetworkTestError::Connection(format!("Failed to connect to DNS server: {}", e)))?;
        ...
        if bytes_read < 12 {
            return Err(NetworkTestError::Connection("Invalid DNS response".to_string()));
        }
        
        let response_code = response_buffer[3] & 0x0F;
        if response_code != 0 {
            return Err(NetworkTestError::Connection(format!("DNS query failed with code: {}", response_code)));
        }
    ...
    fn print_results(&self, result: &DnsStabilityResult) {
        ...
        println!("  Successful: {} ({:.1}%)", result.successful_queries, result.success_rate);
        println!("  Failed: {} ({:.1}%)", 
        ...
        println!("DNS Stability Score: {:.1}/100", dns_score);
    }
    ...
    fn print_dns_analysis(&self, result: &DnsStabilityResult) {
        ...
        if timeout_rate <= 1.0 {
            println!("  ✓ Timeout Rate: Excellent ({:.1}%)", timeout_rate);
        } else if timeout_rate <= 5.0 {
            println!("  ⚠ Timeout Rate: Acceptable ({:.1}%)", timeout_rate);
        } else {
            println!("  ✗ Timeout Rate: High ({:.1}%)", timeout_rate);
        }
    ...
    fn calculate_domain_consistency(&self, result: &DnsStabilityResult) -> f64 {
        ...
        let success_rates: Vec<f64> = result.domain_results.values()
            .map(|dr| if dr.total_queries > 0 {
    ...
    fn calculate_dns_score(&self, result: &DnsStabilityResult) -> f64 {
        ...
        (success_score * 0.4 + speed_score * 0.3 + timeout_score * 0.2 + consistency_score * 0.1)
            .max(0.0).min(100.0)
    }
