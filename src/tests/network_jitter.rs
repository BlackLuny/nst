...
impl NetworkJitterTest {
    pub fn new(proxy_addr: &str, targets: Vec<String>, ping_interval_ms: u64, test_duration_sec: u64) -> Self {
        Self {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("Ping interval: {:?}, Test duration: {:?}", self.ping_interval, self.test_duration);
    ...
    async fn run_jitter_test(&self, client: &Socks5Client) -> Result<NetworkJitterResult> {
        ...
        for target in &self.targets {
            target_results.insert(target.clone(), TargetJitterResult {
                target: target.clone(),
                total_pings: 0,
                successful_pings: 0,
                failed_pings: 0,
        ...
        while Instant::now() < end_time {
            ...
            match self.perform_ping(client, target).await {
                Ok(PingResult { success: true, rtt: Some(rtt), .. }) => {
                    successful_pings += 1;
                ...
                Ok(PingResult { success: false, error: Some(error), .. }) => {
        ...
        let min_rtt = all_rtt_samples.iter().min().copied().unwrap_or(Duration::ZERO);
        let max_rtt = all_rtt_samples.iter().max().copied().unwrap_or(Duration::ZERO);
    ...
    async fn tcp_ping_via_proxy(&self, client: &Socks5Client, target: &str) -> Result<()> {
        let mut stream = client.connect(target).await
            .map_err(|e| NetworkTestError::Connection(format!("Failed to connect to target: {}", e)))?;
        ...
        let bytes_read = timeout(Duration::from_millis(1000), stream.read(&mut response_buffer)).await
    ...
    fn calculate_jitter(&self, rtt_samples: &[Duration]) -> Duration {
        ...
        for i in 1..rtt_samples.len() {
            let diff = if rtt_samples[i] > rtt_samples[i - 1] {
                rtt_samples[i] - rtt_samples[i - 1]
            } else {
                rtt_samples[i - 1] - rtt_samples[i]
            };
            jitter_sum += diff;
            jitter_count += 1;
        }
    ...
    fn print_results(&self, result: &NetworkJitterResult) {
        ...
        for (target, target_result) in sorted_targets {
            println!("  {:25} | {:5} | {:6.1}% | {:4.1}% | {:6.0}ms | {:5.0}ms",
        ...
        println!("Network Quality Score: {:.1}/100", network_score);
    }
    ...
    fn print_network_quality_analysis(&self, result: &NetworkJitterResult) {
        ...
        if result.packet_loss_rate <= 0.1 {
            println!("  ✓ Packet Loss: Excellent ({:.2}%)", result.packet_loss_rate);
    ...
    fn calculate_latency_consistency(&self, result: &NetworkJitterResult) -> f64 {
        ...
        let variance = result.rtt_samples.iter()
            .map(|rtt| (rtt.as_secs_f64() - mean_rtt).powi(2))
            .sum::<f64>() / result.rtt_samples.len() as f64;
    ...
    fn calculate_target_consistency(&self, result: &NetworkJitterResult) -> f64 {
        ...
        let avg_rtts: Vec<f64> = result.target_results.values()
        ...
        let variance = avg_rtts.iter()
            .map(|&rtt| (rtt - mean_avg_rtt).powi(2))
            .sum::<f64>() / avg_rtts.len() as f64;
    ...
    fn calculate_network_quality_score(&self, result: &NetworkJitterResult) -> f64 {
        ...
        (packet_loss_score * 0.3 + latency_score * 0.3 + jitter_score * 0.25 + consistency_score * 0.15)
            .max(0.0).min(100.0)
    }
