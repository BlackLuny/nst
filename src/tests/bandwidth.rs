...
impl BandwidthTest {
    pub fn new(proxy_addr: &str, target_addr: &str, chunk_size: usize, test_duration_sec: u64) -> Self {
        Self {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("Chunk size: {} bytes, Test duration: {:?}", self.chunk_size, self.test_duration);
    ...
    async fn run_bandwidth_test(&self, client: &Socks5Client) -> Result<BandwidthResult> {
        ...
        while Instant::now() < end_time {
            ...
            match self.perform_data_transfer(&mut stream, &mut result, chunk_start).await {
                Ok(_) => {
    ...
    fn print_results(&self, result: &BandwidthResult) {
        ...
        println!("  Total Bytes Sent: {} ({:.2} MB)", 
                 result.total_bytes_sent,
                 result.total_bytes_sent as f64 / 1_048_576.0);
        println!("  Total Bytes Received: {} ({:.2} MB)", 
                 result.total_bytes_received,
                 result.total_bytes_received as f64 / 1_048_576.0);
        println!();
        
        println!("Speed Statistics:");
        println!("  Average Upload Speed: {:.2} KB/s ({:.2} Mbps)", 
                 result.average_upload_speed / 1024.0,
                 (result.average_upload_speed * 8.0) / 1_000_000.0);
        println!("  Average Download Speed: {:.2} KB/s ({:.2} Mbps)", 
                 result.average_download_speed / 1024.0,
                 (result.average_download_speed * 8.0) / 1_000_000.0);
        
        if !result.upload_samples.is_empty() {
            let max_upload = result.upload_samples.iter()
                .map(|s| s.bytes_per_second)
                .fold(0.0f64, f64::max);
            let min_upload = result.upload_samples.iter()
                .map(|s| s.bytes_per_second)
                .fold(f64::INFINITY, f64::min);
            
            println!("  Upload Speed Range: {:.2} - {:.2} KB/s", 
                     min_upload / 1024.0, max_upload / 1024.0);
        }
        
        if !result.download_samples.is_empty() {
            let max_download = result.download_samples.iter()
                .map(|s| s.bytes_per_second)
                .fold(0.0f64, f64::max);
            let min_download = result.download_samples.iter()
                .map(|s| s.bytes_per_second)
                .fold(f64::INFINITY, f64::min);
            
            println!("  Download Speed Range: {:.2} - {:.2} KB/s", 
                     min_download / 1024.0, max_download / 1024.0);
        }
        println!();
        
        println!("Connection Quality:");
        println!("  Connection Interruptions: {}", result.connection_interruptions);
        ...
        let stability_score = if total_samples > 0 {
            let connection_stability = if result.connection_interruptions == 0 { 1.0 } else {
                1.0 / (1.0 + result.connection_interruptions as f64 * 0.2)
            };
            let integrity_score = 1.0 - (error_rate / 100.0);
            (connection_stability * integrity_score * 100.0).max(0.0).min(100.0)
        } else {
            0.0
        };
        
        println!("  Bandwidth Stability Score: {:.1}/100", stability_score);
        println!();
        
        let speed_consistency = if result.upload_samples.len() > 1 {
            let mean = result.average_upload_speed;
            let variance: f64 = result.upload_samples.iter()
                .map(|s| (s.bytes_per_second - mean).powi(2))
                .sum::<f64>() / result.upload_samples.len() as f64;
        ...
        if !result.upload_samples.is_empty() && !result.download_samples.is_empty() {
            let total_transfer_time: Duration = result.upload_samples.iter()
                .chain(result.download_samples.iter())
                .map(|s| s.duration)
                .sum();
            let avg_transfer_time = total_transfer_time / (result.upload_samples.len() + result.download_samples.len()) as u32;
