...
impl ConnectionPerfTest {
    ...
    pub async fn run(&self) -> Result<()> {
        ...
        info!("Concurrent: {}, Total: {}", self.concurrent_connections, self.total_connections);
    ...
    async fn run_connection_perf_test(&self, client: &Socks5Client) -> Result<ConnectionPerfResult> {
        ...
        let connection_times: Vec<Duration> = sequential_results.iter()
            .filter(|r| r.success)
            .map(|r| r.total_time)
            .collect();
        
        let socks5_handshake_times: Vec<Duration> = sequential_results.iter()
            .filter_map(|r| r.socks5_time)
            .collect();
        
        let target_connect_times: Vec<Duration> = sequential_results.iter()
        ...
        let min_connection_time = connection_times.iter().min().copied().unwrap_or(Duration::ZERO);
        let max_connection_time = connection_times.iter().max().copied().unwrap_or(Duration::ZERO);
    ...
    async fn run_sequential_test(&self, client: &Socks5Client) -> Vec<ConnectionAttempt> {
        ...
        for i in 0..self.total_connections {
            debug!("Sequential connection attempt {}/{}", i + 1, self.total_connections);
    ...
    async fn run_concurrent_tests(&self, client: &Socks5Client) -> Vec<ConcurrentTestResult> {
        ...
        for &concurrent_level in &concurrent_levels {
            ...
            let successful = concurrent_results.iter()
                .filter(|r| r.is_ok() && r.as_ref().unwrap().success)
                .count();
            let failed = concurrent_level - successful;
            
            let successful_times: Vec<Duration> = concurrent_results.iter()
    ...
    async fn attempt_single_connection_static(client: &Socks5Client, target_addr: &str) -> ConnectionAttempt {
    ...
    fn print_results(&self, result: &ConnectionPerfResult) {
        ...
        println!("  Target Concurrent Connections: {}", self.concurrent_connections);
        ...
        if !result.concurrent_test_results.is_empty() {
            ...
            for test_result in &result.concurrent_test_results {
                let success_rate = if test_result.concurrent_level > 0 {
                    test_result.successful_connections as f64 / test_result.concurrent_level as f64 * 100.0
                } else {
                    0.0
                };
                
                println!("  {:5} | {:7} | {:6} | {:9.1}% | {:8.0}ms | {:9.0}ms",
    ...
    fn print_performance_analysis(&self, result: &ConnectionPerfResult) {
        ...
        if let Some(best_concurrent) = result.concurrent_test_results.iter()
            .filter(|r| r.successful_connections == r.concurrent_level)
            .max_by_key(|r| r.concurrent_level) {
            println!("  ✓ Max reliable concurrent connections: {}", best_concurrent.concurrent_level);
        }
    ...
    fn calculate_performance_score(&self, result: &ConnectionPerfResult) -> f64 {
        ...
        let concurrent_score = if let Some(max_concurrent) = result.concurrent_test_results.iter()
            .filter(|r| r.successful_connections == r.concurrent_level)
        ...
        (success_score * 0.4 + speed_score * 0.3 + consistency_score * 0.2 + concurrent_score * 0.1)
            .max(0.0).min(100.0)
    }
}